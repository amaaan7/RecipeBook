...
export default function EditRecipePage() {
    ...
    // Pre-fill form once recipe data loads
    useEffect(() => {
        if (!recipe) return;
        setForm({
            title: recipe.title,
        ...
        if (recipe.ingredients?.length) setIngredients(recipe.ingredients.map(i => ({ amount: i.amount, name: i.name })));
        if (recipe.steps?.length) setSteps(recipe.steps.map(s => ({ step_number: s.step_number, instruction: s.instruction })));
    }, [recipe]);

    const mutation = useMutation({
        mutationFn: (data) => api.put(`/api/recipes/${id}/`, data),
        onSuccess: () => {
    ...
    const handleSubmit = (e) => {
        ...
        mutation.mutate({ ...form, image: imageUrl, ingredients: filledIngredients, steps: filledSteps });
    };
    ...
    if (isLoading) return <div className="flex justify-center py-20 text-gray-400">Loading recipe...</div>;
